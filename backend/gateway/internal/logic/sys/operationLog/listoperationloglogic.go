...
type (
	...
	ListLibResp struct {
		result.PageResult
		List []models.SysOperationLog `json:"list"`
	}
)
...
func (l *ListOperationLogLogic) ListOperationLog(req *ListOperationLogReq) (*ListLibResp, error) {
	db := l.svcCtx.DB.Model(&models.SysOperationLog{})
	//搜索
	if req.Keyword != "" {
		db = db.Where("module LIKE ? OR action LIKE ?", "%"+req.Keyword+"%","%"+req.Keyword+"%")
	}
	// 分页
	...
	// 查询列表
	var list []models.SysOperationLog
